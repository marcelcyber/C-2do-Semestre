...
double Operacion_2();
double Operacion_3();
double Calculo_Hipotenusa();
double Calculo_Distancia();
double Calculo_Area();

int main(){

double Ox,Oy,Px,Py,Qx,Qy,da,db,dc,h1,a,b,c,x,As,A;  
...
double Calculo_Hipotenusa(double h1, double a, double b, double c){
h1 = a*b/c; 
return h1;
}
double Calculo_Distancia(double x, double a,double h1){
x = sqrt(pow(a,2)-pow(h1,2));
return x;
}

double Calculo_Area(double As,double h1,double x,double a, double b, double c){
As =  (Calculo_Hipotenusa(h1,a, b, c)*Calculo_Distancia( x, a, h1))/2; 
return As;
}
